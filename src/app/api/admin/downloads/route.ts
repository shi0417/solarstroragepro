...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (search) {
      ...
      whereClause = `WHERE email ILIKE '%${escaped}%' OR name ILIKE '%${escaped}%' OR company ILIKE '%${escaped}%' OR resource_name ILIKE '%${escaped}%'`;
    }

    const data = await querySupabase(
      `SELECT * FROM public.resource_downloads ${whereClause} ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset};`
    );

    const countResult = await querySupabase(
      `SELECT count(*)::int AS total FROM public.resource_downloads ${whereClause};`
    );
