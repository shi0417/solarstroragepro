...
slug = "energy-storage-fire-safety-standards"
date = "2026-07-04"
read_time = 10
tags = ["Fire Safety", "BESS", "Energy Storage", "IEC 62619", "NFPA 855", "Safety Standards"]
...
print("=== Checking existing slugs ===")
existing = supabase_get("blog_articles?select=slug")
...
print(f"Slug '{slug}' is available. Proceeding...")

# Step 2: Insert article
print("\n=== Inserting blog article ===")
article_payload = {
    "slug": slug,
    "date": date,
    "read_time": read_time,
...
article_result = supabase_insert("blog_articles", article_payload)
article_id = article_result[0]["id"] if isinstance(article_result, list) else article_result["id"]
print(f"Article ID: {article_id}")

# ========== EN BODY ==========
en_body = """## Why Fire Safety Is the #1 Concern for BESS Projects
...
# Step 4: Verify
print("\n=== Verifying ===")
verify = supabase_get(f"blog_articles?slug=eq.{slug}&select=id,slug,date")
print(f"Verification: {json.dumps(verify, ensure_ascii=False)}")

verify_translations = supabase_get(f"blog_article_translations?article_id=eq.{article_id}&select=locale,category,title")
for t in verify_translations:
    print(f"  [{t['locale']}] {t['category']} | {t['title'][:80]}...")

print(f"\n=== DONE: Article #{len(existing_slugs)+1} published successfully! ===")
print(f"Slug: {slug}")
